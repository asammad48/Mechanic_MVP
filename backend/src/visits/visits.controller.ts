...
export const exportVisits = async (req: Request, res: Response) => {
  try {
    ...
    worksheet.columns = [
      ...
      { header: 'Visit ID', key: 'id', width: 20 },
    ...
    visits.forEach(visit => {
      worksheet.addRow({
        ...
        id: visit.id,
        regNo: visit.vehicle.regNo,
        vehicle: `${visit.vehicle.make} ${visit.vehicle.model}`,
        customer: visit.customer.name,
        phone: visit.customer.phone,
        status: visit.status,
        total: Number(visit.grandTotal),
        paid: Number(visit.paidAmount),
        due: Number(visit.dueAmount),
        mechanic: visit.assignedMechanic?.name || 'N/A'
      });
    });
    ...
    res.status(500).json({ message: 'Failed to export visits report' });
  }
};
